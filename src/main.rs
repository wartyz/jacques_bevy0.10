...
pub fn main() {
    App::new()
        ...
        // El modo turbofish agrega el recurso con los valores default
        .init_resource::<Score>()
...
pub fn update_enemy_direction(
    ...
    let half_enemy_size = ENEMY_SIZE / 2.; // 32.
    ...
    for (transform, mut enemy) in enemy_query.iter_mut() {
        ...
        if direction_changed {
            ...
            // Play aleatoriamente uno de los dos efectos de sonido
...
pub fn confine_enemy_movement(
    ...
    for mut transform in enemy_query.iter_mut() {
        ...
        // Limita el enemigo en la posición x
        if translation.x < x_min {
            translation.x = x_min;
        } else if translation.x > x_max {
            translation.x = x_max;
        ...
        // Limita el enemigo en la posición y
        if translation.y < y_min {
            translation.y = y_min;
        } else if translation.y > y_max {
            translation.y = y_max;
    ...
    if let Ok((player_entity, player_transform)) = player_query.get_single_mut() {
        for enemy_transform in enemy_query.iter() {
            ...
            if distance < player_radius + enemy_radius {
                println!("Enemigo choca con Player, final de partida");
                let sound_effect = asset_server.load("audio/explosionCrunch_000.ogg");
                audio.play(sound_effect);
                commands.entity(player_entity).despawn(); // Desaparece Entity
...
pub fn handle_game_over(
    ...
    for event in game_over_event_reader.iter() {
        println!("Tu puntuación final es: {}", event.score.to_string());
    }
}
...
pub fn high_scores_updated(
    ...
    if high_scores.is_changed() {
        println!("Puntuación más alta: {:?}", high_scores);
    }
}